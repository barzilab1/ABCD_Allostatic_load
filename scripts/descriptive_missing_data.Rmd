...
missing_main <- dataset_main %>% select(all_of(vars)) %>% 
  ...
  mutate(missing_main = paste0(n_miss, " (", pct_miss, "%)")) %>% 
  select(-n_miss, -pct_miss)

missing_whole <- dataset_2y %>% select(all_of(vars)) %>% 
  miss_var_summary() %>% 
  mutate(pct_miss = round(pct_miss, 2)) %>%
  mutate(missing_whole = paste0(n_miss, " (", pct_miss, "%)")) %>% 
  select(-n_miss, -pct_miss)
  
tableS1 <- missing_main %>% left_join(missing_whole)
write.csv(tableS1, "../outputs_paper/table_S1.csv")
```
