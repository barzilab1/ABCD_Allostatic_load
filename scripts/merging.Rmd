...
```{r}
# Check the list of participants with errors in measures of weight and height (ABCD 5.0) # at 2-year FU only
IDs_error <- c("NDAR_INVM2AA5DTD", "NDAR_INVVMT7W436", "NDAR_INVVDAGZAU0", "NDAR_INV39Z1U19N", "NDAR_INVLEXJMGJ6")

# Check of all those kids were included in the allostatic load data
table(IDs_error %in% dataset$src_subject_id[!is.na(dataset$allostatic_load)]) # 5

# Take those kids out from the data
dataset <- dataset %>% filter(!src_subject_id %in% IDs_error)
```
...
dataset %>% 
filter(if_all(c(biospec_blood_hemoglobin_a1, biospec_blood_cholesterol, biospec_blood_hdl_cholesterol, hormone_scr_dhea_mean), ~ !is.na(.))) %>% 
  dim()
```
